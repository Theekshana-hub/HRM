...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'cancel') {
    ...
    if (!empty($ids)) {
        ...
        $in = implode(',', $ids);
        $conn->query("UPDATE leave_applications SET status = 'Cancelled' WHERE id IN ($in)");
        header("Location: leave-cancel.php?cancelled=1");
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'search') {
    $searched = true;
    $sql = "SELECT * FROM leave_applications WHERE status != 'Cancelled'";
    $params = [];
    $types = '';
    if ($emp !== '') {
...
<title>Leave Cancel | SMART HRIS™</title>
...
.form-card{background:#fff;border:1px solid var(--border);border-radius:12px;padding:24px 28px;margin-bottom:20px;box-shadow:0 1px 3px rgba(0,0,0,.04);max-width:520px}
.form-grid{display:grid;grid-template-columns:110px 1fr;gap:14px 16px;align-items:center}
...
.btn-search{
    ...
    font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;margin-top:18px;transition:all .2s
}
...
<h1 class="page-title">Leave Cancel</h1>

<?php if (isset($_GET['cancelled'])): ?><div class="alert"><i class="fas fa-check-circle"></i> Selected leave(s) cancelled.</div><?php endif; ?>
...
<div class="form-grid">
    ...
    <input type="text" name="employee" value="<?= htmlspecialchars($emp) ?>" placeholder="Employee name or number" required>
...
<tr><td colspan="8" class="empty">No leave applications found for the selected filters.</td></tr>
...
<?php if (!empty($results)): ?>
<button type="submit" class="btn-cancel" onclick="return confirm('Cancel selected leave(s)?')"><i class="fas fa-times-circle"></i> Cancel Selected</button>
<?php endif; ?>
</form>
<?php endif; ?>
</main>
