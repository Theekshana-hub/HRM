...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    $att_group   = $_GET['attendance_group'] ?? '';
    $emp_name    = trim($_GET['employee_name'] ?? '');
    $from_date   = $_GET['from_date'] ?? '';
    $to_date     = $_GET['to_date'] ?? '';

    header('Content-Type: application/vnd.ms-excel');
    header('Content-Disposition: attachment; filename="attendance_shift_report.xls"');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $att_group   = $_POST['attendance_group'] ?? '';
...
$q_report = urlencode($_POST['report'] ?? '');
$q_group  = urlencode($_POST['attendance_group'] ?? '');
$q_emp    = urlencode($_POST['employee_name'] ?? '');
$q_from   = urlencode($_POST['from_date'] ?? '');
$q_to     = urlencode($_POST['to_date'] ?? '');
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Attendance Shift Report | SMART HRIS™</title>
...
.form-row{display:grid;grid-template-columns:130px 280px;gap:12px 16px;align-items:center;margin-bottom:14px}
.form-row.two-col{grid-template-columns:130px 280px 100px 160px;gap:12px 20px}
...
.btn-row{display:flex;align-items:flex-end;gap:24px;margin-top:20px}
.btn-search{
...
<h1 class="page-title">Attendance Shift Report</h1>
...
<form method="POST" action="" id="reportForm">
    <div class="form-row">
        ...
        <select name="report" id="report">
            ...
            <option value="Daily" <?= ($_POST['report'] ?? '') === 'Daily' ? 'selected' : '' ?>>Daily Shift Report</option>
            <option value="Summary" <?= ($_POST['report'] ?? '') === 'Summary' ? 'selected' : '' ?>>Shift Summary</option>
            <option value="Detail" <?= ($_POST['report'] ?? '') === 'Detail' ? 'selected' : '' ?>>Detailed Report</option>
            <option value="Late" <?= ($_POST['report'] ?? '') === 'Late' ? 'selected' : '' ?>>Late Arrival Report</option>
        </select>
    </div>

    <div class="form-row two-col">
        <label>Attendance Group</label>
        <select name="attendance_group" id="attendance_group">
            <option value="">-select-</option>
            <option value="General" <?= ($_POST['attendance_group'] ?? '') === 'General' ? 'selected' : '' ?>>General</option>
            <option value="Shift A" <?= ($_POST['attendance_group'] ?? '') === 'Shift A' ? 'selected' : '' ?>>Shift A</option>
            <option value="Shift B" <?= ($_POST['attendance_group'] ?? '') === 'Shift B' ? 'selected' : '' ?>>Shift B</option>
            <option value="Night Shift" <?= ($_POST['attendance_group'] ?? '') === 'Night Shift' ? 'selected' : '' ?>>Night Shift</option>
        </select>
    ...
    <div class="form-row two-col">
        ...
        <input type="text" name="employee_name" value="<?= htmlspecialchars($_POST['employee_name'] ?? '') ?>" placeholder="Employee name or number">
        <label>To Date</label>
    ...
    <div class="btn-row">
        ...
        <a href="?export=excel&report=<?= $q_report ?>&attendance_group=<?= $q_group ?>&employee_name=<?= $q_emp ?>&from_date=<?= $q_from ?>&to_date=<?= $q_to ?>" class="btn-excel">
...
<tr><td colspan="8"><div class="empty-state"><i class="fas fa-inbox"></i>No records found for the selected filters.</div></td></tr>
...
<td><?= htmlspecialchars($r['attendance_group'] ?? '') ?></td>
</tr>
