...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save') {
    ...
    $sub_department_no = $_POST['sub_department_no'] ?? '';
    $sub_department_name = $_POST['sub_department_name'] ?? '';
    $edit_id = $_POST['edit_id'] ?? '';
    if (!empty($sub_department_name)) {
        if (!empty($edit_id)) {
            $sql = "UPDATE sub_departments SET company_name=?, division_name=?, department_name=?, sub_department_no=?, sub_department_name=? WHERE id=?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssssi", $company_name, $division_name, $department_name, $sub_department_no, $sub_department_name, $edit_id);
        } else {
            $sql = "INSERT INTO sub_departments (company_name, division_name, department_name, sub_department_no, sub_department_name) VALUES (?,?,?,?,?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssss", $company_name, $division_name, $department_name, $sub_department_no, $sub_department_name);
        }
        $stmt->execute(); $stmt->close();
        header("Location: sub-department-registration.php?success=1"); exit;
    }
}
...
$rows=[]; $r=$conn->query("SELECT * FROM sub_departments ORDER BY id ASC"); if($r) while($row=$r->fetch_assoc()) $rows[]=$row;
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Sub Department Registration | SMART HRIS™</title>
...
<h1 class="page-title">Sub Department Registration</h1>
<?php if (isset($_GET['success'])): ?><div class="alert"><i class="fas fa-check-circle"></i> Sub Department saved successfully!</div><?php endif; ?>
<div class="form-card">
<form method="POST">
...
<div class="form-group"><label>Sub Department No</label><input type="text" name="sub_department_no" id="sub_department_no"></div>
...
<div class="form-group"><label>Sub Department Name</label><input type="text" name="sub_department_name" id="sub_department_name" required></div>
...
<table>
<thead><tr><th>Sub Department Name</th><th>Sub Dept No</th><th>Company Name</th><th>Division Name</th><th>Department Name</th><th>Actions</th></tr></thead>
<tbody>
<?php if(empty($rows)): ?><tr><td colspan="6" style="text-align:center;color:#94a3b8;padding:20px">No records yet.</td></tr>
<?php else: foreach($rows as $r): ?>
<tr>
<td><?=htmlspecialchars($r['sub_department_name']??'')?></td>
<td><?=htmlspecialchars($r['sub_department_no']??'')?></td>
...
<script>
function clearForm(){['edit_id','company_name','division_name','department_name','sub_department_no','sub_department_name'].forEach(id=>document.getElementById(id).value='')}
function editRow(r){document.getElementById('edit_id').value=r.id;document.getElementById('company_name').value=r.company_name||'';document.getElementById('division_name').value=r.division_name||'';document.getElementById('department_name').value=r.department_name||'';document.getElementById('sub_department_no').value=r.sub_department_no||'';document.getElementById('sub_department_name').value=r.sub_department_name||'';window.scrollTo({top:0,behavior:'smooth'});}
</script>
</body>
</html>