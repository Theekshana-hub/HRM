...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save') {
    ...
    if (!empty($section_name)) {
        if (!empty($edit_id)) {
            $sql = "UPDATE sections SET company_name=?, division_name=?, department_name=?, sub_department_name=?, section_no=?, section_name=? WHERE id=?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssssi", $company_name, $division_name, $department_name, $sub_department_name, $section_no, $section_name, $edit_id);
        } else {
            $sql = "INSERT INTO sections (company_name, division_name, department_name, sub_department_name, section_no, section_name) VALUES (?,?,?,?,?,?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssss", $company_name, $division_name, $department_name, $sub_department_name, $section_no, $section_name);
        }
        $stmt->execute(); $stmt->close();
        header("Location: section-registration.php?success=1"); exit;
    }
}
...
$subDeptList=[]; $r=$conn->query("SELECT DISTINCT sub_department_name FROM sub_departments ORDER BY sub_department_name"); if($r) while($row=$r->fetch_assoc()) $subDeptList[]=$row['sub_department_name'];
$rows=[]; $r=$conn->query("SELECT * FROM sections ORDER BY id ASC"); if($r) while($row=$r->fetch_assoc()) $rows[]=$row;
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Section Registration | SMART HRIS™</title>
...
<h1 class="page-title">Section Registration</h1>
<?php if (isset($_GET['success'])): ?><div class="alert"><i class="fas fa-check-circle"></i> Section saved successfully!</div><?php endif; ?>
<div class="form-card">
<form method="POST">
...
<div class="form-group"><label>SubDepartment Name</label>
<select name="sub_department_name" id="sub_department_name"><option value="">-select-</option>
<?php foreach($subDeptList as $s): ?><option value="<?=htmlspecialchars($s)?>"><?=htmlspecialchars($s)?></option><?php endforeach; ?>
</select></div>
...
<div class="form-group"><label>Section No</label><input type="text" name="section_no" id="section_no"></div>
...
<div class="form-group"><label>Section Name</label><input type="text" name="section_name" id="section_name" required></div>
</div>
<div class="btn-row">
...
<table>
<thead><tr><th>Section Name</th><th>Section No</th><th>Company Name</th><th>Division Name</th><th>Department Name</th><th>SubDepartment</th><th>Actions</th></tr></thead>
<tbody>
...
<td><?=htmlspecialchars($r['sub_department_name']??'')?></td>
...
<script>
function clearForm(){['edit_id','company_name','division_name','department_name','sub_department_name','section_no','section_name'].forEach(id=>document.getElementById(id).value='')}
function editRow(r){document.getElementById('edit_id').value=r.id;document.getElementById('company_name').value=r.company_name||'';document.getElementById('division_name').value=r.division_name||'';document.getElementById('department_name').value=r.department_name||'';document.getElementById('sub_department_name').value=r.sub_department_name||'';document.getElementById('section_no').value=r.section_no||'';document.getElementById('section_name').value=r.section_name||'';window.scrollTo({top:0,behavior:'smooth'});}
</script>
</body>
</html>