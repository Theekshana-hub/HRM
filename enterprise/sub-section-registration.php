...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save') {
    ...
    $sub_section_no = $_POST['sub_section_no'] ?? '';
    $sub_section_name = $_POST['sub_section_name'] ?? '';
    $edit_id = $_POST['edit_id'] ?? '';
    if (!empty($sub_section_name)) {
        if (!empty($edit_id)) {
            $sql = "UPDATE sub_sections SET company_name=?, division_name=?, department_name=?, section_name=?, sub_section_no=?, sub_section_name=? WHERE id=?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssssi", $company_name, $division_name, $department_name, $section_name, $sub_section_no, $sub_section_name, $edit_id);
        } else {
            $sql = "INSERT INTO sub_sections (company_name, division_name, department_name, section_name, sub_section_no, sub_section_name) VALUES (?,?,?,?,?,?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssss", $company_name, $division_name, $department_name, $section_name, $sub_section_no, $sub_section_name);
        }
        $stmt->execute(); $stmt->close();
        header("Location: sub-section-registration.php?success=1"); exit;
    }
}
$companyList=[]; $r=$conn->query("SELECT company_name FROM companies ORDER BY company_name"); if($r) while($row=$r->fetch_assoc()) $companyList[]=$row['company_name'];
$divisionList=[]; $r=$conn->query("SELECT DISTINCT division_name FROM divisions ORDER BY division_name"); if($r) while($row=$r->fetch_assoc()) $divisionList[]=$row['division_name'];
$deptList=[]; $r=$conn->query("SELECT DISTINCT department_name FROM departments ORDER BY department_name"); if($r) while($row=$r->fetch_assoc()) $deptList[]=$row['department_name'];
$sectionList=[]; $r=$conn->query("SELECT DISTINCT section_name FROM sections ORDER BY section_name"); if($r) while($row=$r->fetch_assoc()) $sectionList[]=$row['section_name'];
$rows=[]; $r=$conn->query("SELECT * FROM sub_sections ORDER BY id ASC"); if($r) while($row=$r->fetch_assoc()) $rows[]=$row;
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Sub Section Registration | SMART HRIS™</title>
...
<h1 class="page-title">Sub Section Registration</h1>
<?php if (isset($_GET['success'])): ?><div class="alert"><i class="fas fa-check-circle"></i> Sub Section saved successfully!</div><?php endif; ?>
<div class="form-card">
<form method="POST">
<input type="hidden" name="action" value="save"><input type="hidden" name="edit_id" id="edit_id" value="">
<div class="form-grid">
...
<select name="section_name" id="section_name"><option value="">-select-</option>
<?php foreach($sectionList as $s): ?><option value="<?=htmlspecialchars($s)?>"><?=htmlspecialchars($s)?></option><?php endforeach; ?>
</select></div>
...
<div class="form-group"><label>Sub Section No</label><input type="text" name="sub_section_no" id="sub_section_no"></div>
<div class="form-group"><label>Department Name</label>
<select name="department_name" id="department_name"><option value="">-select-</option>
<?php foreach($deptList as $d): ?><option value="<?=htmlspecialchars($d)?>"><?=htmlspecialchars($d)?></option><?php endforeach; ?>
</select></div>
<div class="form-group"><label>Sub Section Name</label><input type="text" name="sub_section_name" id="sub_section_name" required></div>
</div>
<div class="btn-row">
...
<table>
<thead><tr><th>Sub Section Name</th><th>Sub Section No</th><th>Company Name</th><th>Division Name</th><th>Department Name</th><th>Section Name</th><th>Actions</th></tr></thead>
<tbody>
<?php if(empty($rows)): ?><tr><td colspan="7" style="text-align:center;color:#94a3b8;padding:20px">No records yet.</td></tr>
<?php else: foreach($rows as $r): ?>
<tr>
<td><?=htmlspecialchars($r['sub_section_name']??'')?></td>
<td><?=htmlspecialchars($r['sub_section_no']??'')?></td>
...
<script>
function clearForm(){['edit_id','company_name','division_name','department_name','section_name','sub_section_no','sub_section_name'].forEach(id=>document.getElementById(id).value='')}
function editRow(r){document.getElementById('edit_id').value=r.id;document.getElementById('company_name').value=r.company_name||'';document.getElementById('division_name').value=r.division_name||'';document.getElementById('department_name').value=r.department_name||'';document.getElementById('section_name').value=r.section_name||'';document.getElementById('sub_section_no').value=r.sub_section_no||'';document.getElementById('sub_section_name').value=r.sub_section_name||'';window.scrollTo({top:0,behavior:'smooth'});}
</script>
</body>
</html>