...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save') {
    ...
    if (!empty($sub_division_name)) {
        if (!empty($edit_id)) {
            $sql = "UPDATE sub_divisions SET company_name=?, division_name=?, sub_division_no=?, sub_division_name=? WHERE id=?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssi", $company_name, $division_name, $sub_division_no, $sub_division_name, $edit_id);
        } else {
            $sql = "INSERT INTO sub_divisions (company_name, division_name, sub_division_no, sub_division_name) VALUES (?,?,?,?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssss", $company_name, $division_name, $sub_division_no, $sub_division_name);
        }
        $stmt->execute();
        $stmt->close();
        header("Location: sub-division-registration.php?success=1");
        exit;
    }
}

$companyList = [];
$r = $conn->query("SELECT company_name FROM companies ORDER BY company_name");
if ($r) while ($row = $r->fetch_assoc()) $companyList[] = $row['company_name'];

$divisionList = [];
$r = $conn->query("SELECT DISTINCT division_name FROM divisions ORDER BY division_name");
if ($r) while ($row = $r->fetch_assoc()) $divisionList[] = $row['division_name'];

$rows = [];
$r = $conn->query("SELECT * FROM sub_divisions ORDER BY id ASC");
...
<title>Sub Division Registration | SMART HRIS™</title>
...
.form-grid{display:grid;grid-template-columns:180px 1fr;gap:12px 16px;align-items:center;max-width:520px}
...
<h1 class="page-title">Sub Division Registration</h1>
<?php if (isset($_GET['success'])): ?><div class="alert"><i class="fas fa-check-circle"></i> Sub Division saved successfully!</div><?php endif; ?>
<div class="form-card">
<form method="POST">
...
<label>Sub Division No</label><input type="text" name="sub_division_no" id="sub_division_no">
<label>Sub Division Name</label><input type="text" name="sub_division_name" id="sub_division_name" required>
</div>
<div class="btn-row">
...
<table>
<thead><tr><th>SubDivision Name</th><th>SubDivision No</th><th>Company Name</th><th>Division Name</th><th>Actions</th></tr></thead>
<tbody>
<?php if (empty($rows)): ?>
<tr><td colspan="5" style="text-align:center;color:#94a3b8;padding:20px">No records yet.</td></tr>
<?php else: foreach ($rows as $r): ?>
<tr>
<td><?= htmlspecialchars($r['sub_division_name'] ?? '') ?></td>
<td><?= htmlspecialchars($r['sub_division_no'] ?? '') ?></td>
...
<script>
function clearForm(){['edit_id','company_name','division_name','sub_division_no','sub_division_name'].forEach(id=>document.getElementById(id).value='')}
function editRow(r){
...
document.getElementById('sub_division_no').value=r.sub_division_no||'';
